...
function doEdit(id){ 
	window.location=_path+'/law/lawType.do?action=edit&lawTypeId='+id;
} function callfunc(functype){	
	
	var lawType={ 
		lawTypeType:document.getElementById("lawTypeType").value,
		lawTypeName:document.getElementById("lawTypeName").value
	};
	var paging={
	...
	ExciseLawAjax.searchLawTypeList(lawType , paging,{
		 callback:function(dataFromServer){
            if(dataFromServer!=null && dataFromServer.length>0){ 
				 var lawTypeList=dataFromServer[0]; 
				 for(var i=0;i<lawTypeList.length;i++){
					 oTable.fnAddData([lawTypeList[i].lawTypeType?lawTypeList[i].lawTypeType=='E'?"กฎหมายสรรพสามิต":"กฎหมายอื่น":"",
									 	lawTypeList[i].lawTypeName?lawTypeList[i].lawTypeName:"",
										lawTypeList[i].lawTypeOrder?lawTypeList[i].lawTypeOrder:"",
					                   	"<span style=\"cursor: pointer, align: center;\" title=\"ดูรายละเอียดข้อมูล\"  onclick=\"return doEdit("+lawTypeList[i].lawTypeId+")\"><img  border=\"0\" src=\""+_path+"/images/icon_15.gif\"/></span>"
					                   ]);
				 }
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	<tr>
		<td height="30" class="head2"><strong>ข้อมูลหลัก >> ประเภทกฎหมาย >> </strong><font color="#000000">ค้นหาข้อมูล </font></td>
		<td align="right">
			<c:if test="${masterLawForm.add=='true'}">
				<INPUT TYPE="button" value="เพิ่มข้อมูล" style="width: 80px" onClick="javascript:window.location='<spring:url value="/law/lawType.do?action=add" htmlEscape="true" />'" >
			</c:if>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">          
				<tr>
					...
					<td width="62%" class="text">&nbsp;
						<form:select path="tmLawType.lawTypeType" id="lawTypeType" cssStyle="width: 250px" cssClass="dropdown">
				...
				<tr>
					<td width="38%" align="right" height="25" class="Ghead">ชื่อประเภทกฎหมาย&nbsp;</td>
					<td width="62%" class="text">&nbsp;
						<form:input path="tmLawType.lawTypeName" id="lawTypeName" cssClass="text" cssStyle="width: 250px" maxlength="100"/>
					</td>
				</tr>
			</table>
        </td>
		...
		<table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
			<thead>
				<tr>
					...
					<th width="*" name="th_type" class="Ghead" align="left">ประเภทกฎหมาย</th>
