...
function doEdit(id){ 
	window.location=_path+'/law/statue.do?action=edit&statueId='+id;
} function callfunc(functype){	
	
	var statue={ 
		statueType:document.getElementById("statueType").value,
		statueName:document.getElementById("statueName").value,
		statueStatus:document.getElementById("statueStatus").value
	};
	var paging={
	...
	ExciseLawAjax.searchStatueList(statue , paging,{
		 callback:function(dataFromServer){
            if(dataFromServer!=null && dataFromServer.length>0){ 
				 var statueList=dataFromServer[0]; 
				 for(var i=0;i<statueList.length;i++){
					 oTable.fnAddData([statueList[i].statueType?statueList[i].statueType=='E'?"กฎหมายสรรพสามิต":"กฎหมายอื่น":"",
							 			statueList[i].statueName?statueList[i].statueName:"",
							 			statueList[i].statueStatus?statueList[i].statueStatus=='A'?"มีผลบังคับใช้":"ไม่มีผลบังคับใช้":"",
										statueList[i].statueOrder?statueList[i].statueOrder:"",
					                   	"<span style=\"cursor: pointer, align: center;\" title=\"ดูรายละเอียดข้อมูล\"  onclick=\"return doEdit("+statueList[i].statueId+")\"><img  border=\"0\" src=\""+_path+"/images/icon_15.gif\"/></span>"
					                   ]);
				 }
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	<tr>
		<td height="30" class="head2"><strong>ข้อมูลหลัก >> พระราชบัญญัติ >> </strong><font color="#000000">ค้นหาข้อมูล </font></td>
		<td align="right">
			<c:if test="${masterLawForm.add=='true'}">
				<INPUT TYPE="button" value="เพิ่มข้อมูล" style="width: 80px" onClick="javascript:window.location='<spring:url value="/law/statue.do?action=add" htmlEscape="true" />'" >
			</c:if>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">			
	...
	<tr>
		<td>
			<table width="100%" border="0" cellspacing="0" cellpadding="0">          
				...
				<tr>
					<td width="38%" align="right" height="25" class="Ghead">ชื่อพระราชบัญญัติ&nbsp;</td>                        
					<td width="62%" height="25">&nbsp;
						<form:input path="tmStatue.statueName" id="statueName" cssClass="text" cssStyle="width: 200px" maxlength="100"/>
					</td>
				</tr>   
				<tr>
					<td width="38%" align="right" height="25" class="Ghead">สถานะ&nbsp;</td>
					<td width="62%" class="text">&nbsp;
	...
	<tr>
		...
		<table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
			<thead>
				<tr>
					<th width="15%" name="th_type" class="Ghead" align="left">กฎหมาย</th>
					<th width="*" name="th_type" class="Ghead" align="left">ชื่อพระราชบัญญัติ</th>
					<th width="10%" name="th_type" class="Ghead" align="left">สถานะ</th>
					<th width="15%" name="th_type" class="Ghead" align="left">ลำดับการแสดงผล</th>
