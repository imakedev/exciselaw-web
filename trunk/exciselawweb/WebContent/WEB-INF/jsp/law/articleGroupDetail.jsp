...
function callfunc(functype)	{
	...
	if(functype!="doDelete"){	
		if(document.getElementById("articleGroupType").value == ""){
			msg += "- กรุณาเลือกกฎหมาย\n";
		}	
		if(document.getElementById("statueId").value == ""){
			msg += "- กรุณาเลือกพระราชบัญญัติ\n";
		}	
		if(document.getElementById("articleGroupName").value == ""){
			msg += "- กรุณากรอกหมวด\n";
		}	
...
<form:hidden path="tmArticleGroup.articleGroupId"/>
<form:hidden path="tmArticleGroup.articleGroupOrder"/>
<table width="100%" border="0" cellpadding="0" cellspacing="0">	
	<tr>
		<td height="30" class="head2"><strong>ข้อมูลหลัก >> หมวด >> </strong><font color="#000000">รายละเอียดข้อมูล</font></td>
		<td align="right">
			<c:if test="${masterLawForm.view=='true'}">
				<INPUT TYPE="button" value="ย้อนกลับ" style="width: 80px" onClick="javascript:window.location='<spring:url value="/law/articleGroup.do?action=search" htmlEscape="true" />'" >
			</c:if>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">			
	...
	<tr>
		...
		<td width="80%" class="text" align="left">
			<form:select path="tmArticleGroup.articleGroupType" id="articleGroupType" cssStyle="width: 200px" cssClass="dropdown">
	...
	<tr>
		...
		<td width="80%" class="text">
			<form:select path="tmArticleGroup.statueId" id="statueId" cssClass="dropdown">
			    <form:option value="" label="- - - Please Select - - -"/> 
	...
	<tr>
		<td width="20%" class="text"><div align="right">หมวด&nbsp;</div></td>
		<td width="80%" class="text">
			<form:input path="tmArticleGroup.articleGroupName" id="articleGroupName" cssClass="text" size="50" maxlength="200"/>
		</td>
	</tr>
	<tr>
		<td width="20%" class="text" valign='top'><div align="right">แทรกตามหมวด&nbsp;</div></td>
		<td width="80%" class="text" align="left">
			<form:select path="articleGroupOrder" id="articleGroupOrder" cssClass="dropdown">
			    <form:option value="" label="- - - Please Select - - -"/> 
    			<form:options items="${articleGroupOrderList}" itemValue="articleGroupOrder" itemLabel="articleGroupName"/> 
			</form:select>
		</td>
	</tr>
	...
	<tr>
		...
		<td class="text">
			<form:input path="tmArticleGroup.createdBy" id="createdBy" cssClass="text" size="20" disabled="true"/>
		</td>
	</tr>
	<tr>
...
function statusMessage(msg,mode){
	alert(msg);
	if(mode=='doSave'){
		document.getElementById("articleGroupType").value="";
		document.getElementById("statueId").value="";
		document.getElementById("articleGroupName").value="";
		document.getElementById("articleGroupOrder").value="";
	}	
}
</script>
