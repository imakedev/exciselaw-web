...
{ 
	...
	window.location=_path+'/information_law/SubMatr.do?action=edit&submatrId='+_id;
	//alert(_id);
...
{	 
	...
	var msSubmatr={
			name:document.getElementById("SUBMATR_NAME").value		 
	};
	var paging={
	...
	ExciseLawAjax.getMsSubmatrs(msSubmatr,paging,{
		 callback:function(dataFromServer){
            if(dataFromServer!=null && dataFromServer.length>0){ 
				 var msSubmatrsList=dataFromServer[0]; 
				 for(var i=0;i<msSubmatrsList.length;i++){
					 oTable.fnAddData([msSubmatrsList[i].name,
					                   msSubmatrsList[i].submatrOrder, 
					                   "<span style=\"cursor: pointer;\" title=\"ดูรายละเอียดข้อมูล\"  onclick=\"return doEditExt("+msSubmatrsList[i].submatrId+")\"><img  border=\"0\" src=\""+_path+"/images/icon_15.gif\"/></span>"
					                   ]);
				 }
...
ObjectControl myObjControl=new ObjectControl();
	...
	String SUBMATR_ID="";
	String SUBMATR_ID_VAL="";
	FUNC = request.getParameter("FUNC");
	SUBMATR_ID_VAL=myStrControl.gNullToBlank(request.getParameter("SUBMATR_ID"));
	...
	sql = " SELECT * FROM MS_SUBMATR";
	sql =sql + " where 1=1 ";
	if(!SUBMATR_NAME.equals("")){
		sql=sql+" and NAME like'%"+SUBMATR_NAME+"%'";
	}
	sql=sql+" order by SUBMATR_ORDER asc";
...
<%--<form id="form1" name="form1" method="post" action="SubMatrList.jsp">
 --%>
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
		...
		<td valign="top">
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
					...
					<td height="25" class="text">
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								...
								<td align="right">
									<INPUT TYPE="button" value="เพิ่มข้อมูล" style="width: 80px" onClick="javascript:window.location='<spring:url value="/information_law/SubMatr.do" htmlEscape="true" />'" >
				...
				<tr><td height="5"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                      ...
                      <tr>
                        <td width="38%" align="right" height="25" class="Ghead">หมวดย่อย&nbsp;</td>                        
                        <td width="62%" height="25">&nbsp;
						<form:input path="msSubmatr.name" id="SUBMATR_NAME" cssClass="text" size="50"/>                        </td>
                      </tr>
                    </table></td>
					</tr>
					<tr><td height='5'>&nbsp;</td></tr>
	...
	<thead>
		<tr>
			<th width="90%" name="th_type" class="Ghead" align="left">หมวดย่อย</th>
...
<%-- 
				  <table align="center"width="98%" border="0" cellspacing="2" cellpadding="0" >
					  ...
					  <td align="center" bgcolor="#999999"></td> 
                        ...
                        <c:forEach items="${msSubmatrs}" var="msSubmatr" varStatus="loop">
                        	<tr>
                        	 <td align="center" ><c:out value="${msSubmatr.name}"/></td> 
					  		 <td align="center" ><c:out value="${msSubmatr.submatrOrder}"/></td>
					  		 <td align="center" >  
					  		 <spring:url value="/information_law/SubMatr.do" var="editUrl">
              						<spring:param name="action" value="edit"/>
              						<spring:param name="submatrId" value="${msSubmatr.submatrId}"/>
          					  </spring:url>
					  		 <span style="cursor: pointer;" title="ดูรายละเอียดข้อมูล"  onclick='javascript:doEdit("${fn:escapeXml(editUrl)}")'><img  border="0" src='/images/icon_15.gif'></span> 
					  		 </td> 
                        	</tr>
                        </c:forEach>
                       </table>
