...
function doEdit(id){ 
	window.location=_path+'/law/articleSection.do?action=edit&articleSectionId='+id;
} function callfunc(functype){	
	
	var articleSection={ 
		articleSectionType:document.getElementById("articleSectionType").value,
		statueId:document.getElementById("statueId").value,
		articleGroupId:document.getElementById("articleGroupId").value,
		articleSectionName:document.getElementById("articleSectionName").value
	};
	var paging={
	...
	ExciseLawAjax.searchArticleSectionList(articleSection , paging,{
		 callback:function(dataFromServer){
            if(dataFromServer!=null && dataFromServer.length>0){ 
				 var articleSectionList=dataFromServer[0]; 
				 for(var i=0;i<articleSectionList.length;i++){
					 oTable.fnAddData([articleSectionList[i].articleSectionTypeDesc?articleSectionList[i].articleSectionTypeDesc:"",
							 			articleSectionList[i].statueName?articleSectionList[i].statueName:"",
									 	articleSectionList[i].articleGroupName?articleSectionList[i].articleGroupName:"",
									 	articleSectionList[i].articleSectionName?articleSectionList[i].articleSectionName:"",
										articleSectionList[i].articleSectionOrder?articleSectionList[i].articleSectionOrder:"",
					                   	"<span style=\"cursor: pointer, align: center;\" title=\"ดูรายละเอียดข้อมูล\"  onclick=\"return doEdit("+articleSectionList[i].articleSectionId+")\"><img  border=\"0\" src=\""+_path+"/images/icon_15.gif\"/></span>"
					                   ]);
				 }
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	<tr>
		<td height="30" class="head2"><strong>ข้อมูลหลัก >> ส่วนที่ >> </strong><font color="#000000">ค้นหาข้อมูล </font></td>
		<td align="right">
			<c:if test="${masterLawForm.add=='true'}">
				<INPUT TYPE="button" value="เพิ่มข้อมูล" style="width: 80px" onClick="javascript:window.location='<spring:url value="/law/articleSection.do?action=add" htmlEscape="true" />'" >
			</c:if>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">          
				<tr>
					...
					<td width="62%" class="text">&nbsp;
						<form:select path="tmArticleSection.articleSectionType" id="articleSectionType" cssStyle="width: 250px" cssClass="dropdown">
				...
				<tr>
					...
					<td width="62%" height="25">&nbsp;
						<form:select path="tmArticleSection.statueId" id="statueId" cssStyle="width: 250px" cssClass="dropdown">
						    <form:option value="" label="- - - Please Select - - -"/> 
				...
				<tr>
					...
					<td width="62%" height="25">&nbsp;
						<form:select path="tmArticleSection.articleGroupId" id="articleGroupId" cssStyle="width: 250px" cssClass="dropdown">
						    <form:option value="" label="- - - Please Select - - -"/> 
				...
				<tr>
					...
					<td width="62%" class="text">&nbsp;
						<form:input path="tmArticleSection.articleSectionName" id="articleSectionName" cssClass="text" cssStyle="width: 250px" maxlength="200"/>
					</td>
				</tr>
			</table>
        </td>
		...
		<table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
			<thead>
				<tr>
					...
					<th width="*" name="th_type" class="Ghead" align="left">ส่วนที่</th>
					<th width="14%" name="th_type" class="Ghead" align="left">ลำดับการแสดงผล</th>
					<th width="8%" name="th_type" class="Ghead" align="left">รายละเอียด</th>
				</tr>
			</thead>
			<tbody  class="text">
			</tbody>	 
