...
function doEdit(id){ 
	window.location=_path+'/law/article.do?action=edit&articleId='+id;
} function callfunc(functype){	

	var article={ 
		statueId:document.getElementById("statueId").value,
		lawTypeId:document.getElementById("lawTypeId").value,
		articleGroupId:document.getElementById("articleGroupId").value,
		articleSectionId:document.getElementById("articleSectionId").value,
	...
	ExciseLawAjax.searchArticleList(article, 1, paging,{
		 callback:function(dataFromServer){
            if(dataFromServer!=null && dataFromServer.length>0){ 
				 var articleList=dataFromServer[0]; 
				 for(var i=0;i<articleList.length;i++){
					 oTable.fnAddData([articleList[i].articleId?articleList[i].articleId:"",
							 			articleList[i].statueName?articleList[i].statueName:"",
									 	articleList[i].lawTypeName?articleList[i].lawTypeName:"",
									 	articleList[i].articleHeaderName?articleList[i].articleHeaderName:"",
									 	articleList[i].articleGroupName?articleList[i].articleGroupName:"",
										articleList[i].articleSectionName?articleList[i].articleSectionName:"",
									 	articleList[i].articleNumber?articleList[i].articleNumber:"",
										articleList[i].articleOrder?articleList[i].articleOrder:"",
					                   	"<span style=\"cursor: pointer, align: center;\" title=\"ดูรายละเอียดข้อมูล\"  onclick=\"return doEdit("+articleList[i].articleId+")\"><img  border=\"0\" src=\""+_path+"/images/icon_15.gif\"/></span>"
					                   ]);
				 }
...
function getExArticleHeaderListByLawTypeId(lawTypeId){
	dwr.util.removeAllOptions("articleHeaderId");   
	dwr.util.addOptions("articleHeaderId", ["- - - Please Select - - -"]);
	if(lawTypeId != ""){
		ExciseLawAjax.getExArticleHeaderListByLawTypeId(lawTypeId, {
			callback:function(data){
				if(data.articleHeaderId != ""){
...
<spring:url value="/law/article.do" var="formAction"></spring:url> 
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	<tr>
		<td height="30" class="head2"><strong>กฎหมายสรรพสามิต >> มาตรากฏหมาย >> </strong><font color="#000000">ค้นหาข้อมูล</font></td>
		<td align="right">
			
				<INPUT TYPE="button" value="เพิ่มข้อมูล" style="width: 80px" onClick="javascript:window.location='<spring:url value="/law/article.do?action=add" htmlEscape="true" />'" >
	...
	<tr>
		<td>
			<table width="100%" border="0" cellspacing="0" cellpadding="0">    
				<tr>
					<td width="20%" align="right" height="25" class="Ghead">พระราชบัญญัติ&nbsp;</td>                        
					<td width="25%" height="25" class="text">&nbsp;
				...
				<tr>
					<td width="20%" align="right" height="25" class="Ghead">ประเภทกฎหมาย&nbsp;</td>                        
					<td width="25%" class="text">&nbsp;
						<form:select path="tsArticle.lawTypeId" id="lawTypeId" cssStyle="width: 300px" cssClass="dropdown"  onchange="getExArticleHeaderListByLawTypeId(this.value);">
						    <form:option value="" label="- - - Please Select - - -"/> 
			    			<form:options items="${lawTypeList}" itemValue="lawTypeId" itemLabel="lawTypeName"/> 
						</form:select>
					</td>
					<td width="15%" align="right" height="25" class="Ghead">ชื่อ พ.ร.บ./พ.ร.ก./พ.ร.ฎ.&nbsp;</td>                        
					<td width="40%" class="text">&nbsp;
						<form:select path="tsArticle.articleHeaderId" id="articleHeaderId" cssStyle="width: 300px" cssClass="dropdown">
						    <form:option value="" label="- - - Please Select - - -"/> 
						</form:select>
					</td>
				</tr>   
				<tr>
					<td width="20%" align="right" height="25" class="Ghead">หมวด&nbsp;</td>                        
					<td width="25%" class="text">&nbsp;
					...
					<td width="15%" align="right" height="25" class="Ghead">ส่วนที่&nbsp;</td>                        
					<td width="40%" class="text">&nbsp;
				...
				<tr>
					<td width="20%" align="right" height="25" class="Ghead">เลขมาตรา&nbsp;</td>
					<td width="25%" class="text">&nbsp;
						<form:input path="tsArticle.articleNumber" id="articleNumber" cssClass="text" cssStyle="width: 250px" maxlength="100"/>
					</td>
					<td width="15%" align="right" height="25" class="Ghead">ลำดับที่&nbsp;</td>                        
					<td width="40%" class="text">&nbsp;
						<form:input path="primaryKey" id="articleId" cssClass="text" cssStyle="width: 150px" maxlength="10"/>
					</td>
				</tr>
			</table>
        </td>
		...
		<table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
			<thead>
				<tr>
					...
					<th width="10%" name="th_type" class="Ghead" align="left">หมวด</th>
					<th width="10%" name="th_type" class="Ghead" align="left">ส่วนที่</th>
					<th width="10%" name="th_type" class="Ghead" align="left">เลขมาตรา</th>
					<th width="10%" name="th_type" class="Ghead" align="left">ลำดับการแสดงผล</th>
